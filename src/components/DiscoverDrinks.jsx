...
import { fetchBarDiscoveryFeed, filterBarPosts, clearBarDiscoveryCache, DISCOVER_CATEGORIES } from '../scrapers/barDiscovery';
import { hapticLight } from '../haptics';
import SafeMediaImage from './SafeMediaImage';
import './DiscoverRecipes.css';

// Bar Discovery: same multi-source blog aggregator, expandable preview cards,
// and post-type filtering as DiscoverRecipes.jsx (Meal Discovery), pointed at
// cocktail/spirits blogs instead. Forked rather than parameterized so the two
// discovery surfaces can be tuned independently -- see barDiscovery.js.
//
// Hands the selected post's link back to the caller exactly like
// DiscoverRecipes does; the caller is responsible for opening the import
// sheet kind-locked to 'drink' (App.jsx's handleQuickImportDrink).
...
export default function DiscoverDrinks({ onClose, onSelectUrl }) {
  // ─── State ───────────────────────────────────────────────────────────
  ...
  const loadFeed = useCallback(async ({ force = false, filter } = {}) => {
    ...
    setError(null);
    try {
      if (force) clearBarDiscoveryCache();
      const filterMode = filter !== undefined ? filter : (showAll ? 'relaxed' : 'strict');
      const data = await fetchBarDiscoveryFeed({ force, filter: filterMode });
      setAllPosts(data.posts || []);
      setSources(data.sources || {});
    } catch (err) {
      console.error('[bar-discover] Feed load failed:', err);
      setError(err.message || 'Failed to load cocktails');
    } finally {
      setLoading(false);
    }
  }, [showAll]);

  // Fetch on mount + re-fetch when filter toggle changes. Single effect
  // avoids the double-fetch race the meal-side component's comment warns
  // about (both fired on mount -> concurrent setLoading calls).
    ...
    >
      ...
      >
        ...
        <div className="discover-header">
          <div>
            <h2 className="discover-title">Discover Cocktails</h2>
            <p className="discover-subtitle">Fresh picks from {Object.keys(sources).length || '4'} cocktail &amp; spirits blogs</p>
          </div>
        ...
        {isOffline ? (
          <div className="discover-offline">
            ...
            <p className="discover-offline-hint">Your saved drinks still work fully offline — this is just for finding new ones.</p>
          </div>
        ) : (
          <>
            {/* ── Search ──────────────────────────────────────────── */}
            ...
            <div className="discover-filter-section">
              <div className="discover-sources" role="tablist" aria-label="Cocktail sources">
                {sourceList.map(src => (
                  <button
                    key={src.key}
                    role="tab"
              ...
              <div className="discover-chips" role="tablist" aria-label="Cocktail categories">
            ...
            <div className="discover-results">
              {loading && (
                <div className="discover-status">
                  ...
                  <span>Finding cocktails from top bar blogs…</span>
                </div>
              )}

              {!loading && error && (
                <div className="discover-status">
                  <span>Couldn't reach cocktail sources right now.</span>
              ...
              {!loading && !error && filteredPosts.length === 0 && (
                <div className="discover-status">
                  {searchText || activeCategory !== 'all' || activeSource !== 'all' ? (
                    <span>No cocktails match your filters — try broadening your search.</span>
                  ) : (
                    <span>No cocktails found right now — try refreshing.</span>
                  )}
                </div>
              )}
              ...
              {!loading && !error && filteredPosts.length > 0 && (
                <div className="discover-footer-note">
                  Showing {filteredPosts.length} cocktail{filteredPosts.length !== 1 ? 's' : ''}
