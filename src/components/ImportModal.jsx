...
export default function ImportModal({ onImport, onClose, title = 'Import Recipe', sharedContent = null }) {
  ...
  async function handleUrlImportV2({ url: rawUrl, onImport, onClose, setError: setErr }) {
    const clean = normalizeInstagramUrl(rawUrl) || rawUrl;
  ...
  const handleUrlImport = async () => {
    ...
    // V2 fast path: Ghost Recipe optimistic UI (feature-flagged)
    if (USE_V2) {
      try {
        await handleUrlImportV2({ url: trimmedUrl, onImport, onClose, setError });
      } catch (err) {
