 ...
 *      Ã¢â‚¬Â¢ Social media URLs Ã¢â€ â€™ headless Chrome (real browser, renders JS)
...
function sanitizeLLMResponse(text) {
  if (!text) return '';
  return text.replace(RE_NULLS, '').trim();
}
...
const NUM_UNIT_RE = /^[\dÃ‚Â½Ã‚Â¼Ã‚Â¾Ã¢â€¦â€œÃ¢â€¦â€Ã¢â€¦â€ºÃ¢â€¦Å“Ã¢â€¦ÂÃ¢â€¦Å¾][\d./\s]*\s*(cups?|tbsp|tsp|tablespoons?|teaspoons?|oz|ounces?|lbs?|pounds?|grams?|kg|ml|cl|liters?|pinch|dash(?:es)?|splash(?:es)?|drops?|parts?|barspoons?|bar spoons?|bunch|cloves?|cans?|jars?|packages?|pkg|sticks?|slices?|handful|sprigs?|heads?|stalks?)/i;
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Caption Parser (4-pass, enhanced for video content) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
// Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Flexible instruction parser Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
function parseRecipeNode(node) {
  ...
  // Directions Ã¢â‚¬â€ Comprehensive parsing:
  ...
  // Image Ã¢â‚¬â€ Pick the best/largest from multiple candidates
...
function parseSpokenTranscript(text) {
  ...
  const QUANTITY_MENTION_RE = /\b(?:cup|tablespoon|teaspoon|tbsp|tsp|ounce|oz|pound|lb|pinch|dash|splash|drop|part|barspoon|handful|bunch|clove|can|jar|package|stick|piece)s?\b/i;
 ...
 * Unified pipeline Ã¢â‚¬â€ tries multiple strategies automatically.
 *
 * Strategy (in order):
...
/**
 * parseVisualJSON Ã¢â‚¬â€ Layout-based recipe extractor.
 *
 * CONTRACT:
...
// ─────────────────────────────────────────────────────────────────────────────
// VIDEO TRANSCRIPTION → RECIPE PIPELINE
// ─────────────────────────────────────────────────────────────────────────────
// Bridges the transcription service (Whisper) with the existing LLM structuring
// pipeline. The transcript is fed into captionToRecipe() exactly like a social
// media caption — Grok/Gemini handle the text→recipe conversion.

/**
 * transcribeVideoForRecipe(url, opts) — full pipeline: transcribe video audio →
 * structure the transcript into a recipe via the LLM pipeline.
 *
 * Used by ImportReview's "Transcribe Video" button and can be wired into
 * importFromInstagram as a fallback when caption extraction fails.
 *
 * @param {string} url - Video URL (IG Reel, TikTok, YouTube, etc.)
 * @param {object} opts
 * @param {function} [opts.onProgress] - (tier, message) callback
 * @param {AbortSignal} [opts.signal]
 * @param {string} [opts.type='meal'] - 'meal' or 'drink'
 * @param {string} [opts.imageUrl] - Existing image URL to preserve
 * @param {string} [opts.model] - Whisper model tier
...
export async function transcribeVideoForRecipe(url, opts = {}) {
  ...
  if (!transcription?.transcript || transcription.transcript.length < 20) {
    onProgress?.('transcribe', 'No usable transcript found');
    return null;
  }

  onProgress?.('transcribe', `Transcript: ${transcription.transcript.length} chars — structuring recipe…`);

  // Route through the existing LLM pipeline
  const recipe = await captionToRecipe(transcription.transcript, {
    title: '',
    imageUrl,
    sourceUrl: url,
    type,
  ...
  if (recipe) {
    recipe._transcriptSource = transcription.extractedVia;
    recipe._structuredVia = `${recipe._structuredVia || 'unknown'}+transcript`;
    recipe.link = recipe.link || url;
    if (imageUrl && !recipe.imageUrl) recipe.imageUrl = imageUrl;
  }
...
/**
 * transcribeFileForRecipe(file, opts) — transcribe a user-uploaded video/audio
 * file and structure the transcript into a recipe.
 *
 * Fully offline path: Web Audio API → Whisper Web Worker → captionToRecipe.
 *
 * @param {File} file - Video or audio file
 * @param {object} opts
 * @param {function} [opts.onProgress]
 * @param {AbortSignal} [opts.signal]
 * @param {string} [opts.type='meal']
 * @param {string} [opts.model] - Whisper model tier
 * @returns {Promise<object|null>}
 */
export async function transcribeFileForRecipe(file, opts = {}) {
  const { onProgress, signal, type = 'meal', model } = opts;

  onProgress?.('transcribe', `Extracting audio from ${file.name}…`);

  const transcription = await transcribeFromFile(file, {
    onProgress,
    signal,
    model,
  });
  ...
  if (!transcription?.transcript || transcription.transcript.length < 20) {
    onProgress?.('transcribe', 'No usable transcript found in file');
    return null;
  }
  ...
  if (recipe) {
    recipe._transcriptSource = transcription.extractedVia;
    recipe._structuredVia = `${recipe._structuredVia || 'unknown'}+file-transcript`;
    recipe._transcriptDuration = transcription.duration;
  }
