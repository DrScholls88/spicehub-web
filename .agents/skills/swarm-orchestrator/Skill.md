...
## Core Logic (RUFLo)
1. **READ**: Analyze the current task and identify which specialized skills from `.Codex/skills/` are required.
2. **UPDATE**: Create or update `.Codex/swarm_state.json` with the minimal code snippets and context needed for the next step.
...
## Memory Structure (.Codex/swarm_state.json)
{
